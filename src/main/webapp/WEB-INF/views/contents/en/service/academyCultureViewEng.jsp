...
app.controller('academyListCtl', function($scope, consts, common) {0
	$scope.loadAcademyList = function($page){
 		...
 		$d = {"baseParms":{"academy_cd": "culture","academy_no": "${academy_no}"},  //
 				"actionList":[
					...
					<div>
						<ul>
							<li ng-repeat="art in academyView" ng-init="{first:$first || $index == 1}">
                                ...
                                <img ng-src="<spring:eval expression="@configure['img.root.path']" />{{art.IMG_VIEW_PATH_EN}}/{{art.VIEW_FILE_NAME_EN}}" alt='AMOUR CULTURE' width="100%"/>
                                </div>
							</li>
						</ul>
					</div>
